...
(_gsScope._gsQueue || (_gsScope._gsQueue = [])).push( function() {
	...
	var	_NaNExp = /[^\d\-\.]/g,
		...
		SnapPlugin = _gsScope._gsDefine.plugin({
			propName: "snap",
			version: "0.0.1",
			API: 2,
			...
			init: function(target, value, tween) {
				...
				var p, s, v, pt, clr1, clr2, rel, i, j, _arr1, _arr2;
				...
				for (p in value) {
					...
					if (p === "fill" || p === "stroke") {
						...
						//points need to be broken into array and tweened independently
						_arr1 = v.split(/[ ,]+/);
						...
						for (i = 0; i < _arr1.length; i += 1) {
							for (j = 0; j < _arr1[i].length; j += 1) {
								...
								if (typeof(_arr1[i][j]) === "number") {
									pt.e = _arr2[i][j];
									pt.s = Number(_arr1[i][j]);
									pt.c = Number(_arr2[i][j]) - pt.s;
								} else {
									pt.s = _arr1[i][j];
									pt.c = _arr1[i][j];
								}

								this._path.push({s: pt.s, c: pt.c});
							}
						}
					
						pt.type = 4; //4 = path tween
					} else {
			...
			set: function(v) {
				var pt = this._firstPT, val, val2, i, e, e2;
					...
					if (!pt.type) {
						...
						for (i = 0; i < this._path.length; i += 1) {
							...
							if (typeof(e.s) === 'number') {
								val = e.s + e.c * v;
								pt.t[pt.p] += String((val < 0.000001 && val > -0.000001) ? 0 : val);
								if (typeof(e2.s) === 'number') { //special case add comma
									val2 = e2.s + e2.c * v;
									if (val2 > -1) {
										pt.t[pt.p] += ",";
									}
